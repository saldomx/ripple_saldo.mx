...
import com.mxinteligente.controller.ventas.AltaProducto.ComboRender;
...
public class Editar extends GenericForwardComposer {
	...
	public void cargarCategoriaProductos(){	
		...
		if(!lstCat.isEmpty()){
			...
			Catproductos categoria = new Catproductos();
			categoria.setId(p.getCategoria().getId());
			categoria = (Catproductos) ventasServ.buscarObjeto(categoria).get(0);
			CmbCategorias.setValue(categoria.getNombre());
	...
	if(CmbCategorias.getSelectedItem()!=null && CmbCategorias.getSelectedItem().getValue()!=null){
		p.getCategoria().setId(((Integer)CmbCategorias.getSelectedItem().getValue()));
	}
//	
		...
		public void show() {
			try {
				Messagebox.show("Desea Eliminar este registro",
						"Confirmación", Messagebox.OK, Messagebox.INFORMATION,
