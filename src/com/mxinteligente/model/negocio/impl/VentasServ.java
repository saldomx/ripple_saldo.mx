...
public class VentasServ extends GenServicio implements VentasServI{
	...
	@Autowired(required=true)
	private CatproductosDaoI catproductosDao;
	...
	public int realizarCompra(Egresos gasto, Usuarios user, String email, Integer pin, Integer idProducto){
		...
		if (lstpro!=null && !lstpro.isEmpty()){
			...
			gasto.setConcepto("POR ADQUISCIÓN DE " + pro.getName());
			...
			if (gasto.getCantidad().compareTo(saldo) == 1) { // No tiene
				...
				if (bUser != null) {
					...
					if(pro.getCategoria()!=null){
						if(pro.getCategoria().getCompany().equals("RECARGA_TELCEL")){
						
							recargaServ.guardarPeticionRecarga(pro, user.getCellPhone());
							status=300; //exito pendiente
						}else if(pro.getCategoria().getCompany().equals("RECARGA_OTROS")){
...
//				    	this.modificarArchivo(pro);
	...
	@Override
	public List obtenerCategoriasProductos() {
		
		return catproductosDao.obtenerCategorias();
	}
